...
$(document).ready(function() {
   ...
   $(window).on('resize orientationchange', function() {
      $('.navSignin').removeClass('is-active');
   });
});
...
$(document).ready(function() {
   var $paraContainer = $(".banner--fixedBackground, .sliderMainImage");
   parallax($paraContainer);
...
$(document).ready(function() {
  ...
  //attach util nav items to the main mobile nav in the drawer
    $('.logoMain').retach({
    destination: '.headerMain-utilInner',
    prependAppend: 'prepend',
    mediaQuery: 641
  });
  $('.navUtil').retach({
...
//    $('.featureBlock--six .featureBlock-figure').scrollFade({
//       testParent: '.featureBlock--six',
//       mediaQuery: 1024
//    });
...
(function($) {
   $.fn.scrollFade = function(opts) {
      ...
      if ($items.size() < 1 || $items.closest(testParent).size() < 1)
        return;
...
//swap background images based on data attributes or on paramaters passed to the function
