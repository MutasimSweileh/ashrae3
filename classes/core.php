...
class core extends mysqli
{
	...
	function yt_exists($my_id, $ar = array())
	{
		if (strpos($my_id, "youtube") !== false) {
			$my_id = explode("v=", $my_id);
			$my_id = $my_id[count($my_id) - 1];
			$my_id = explode("&", $my_id)[0];
		}
		$pra = $ar + array("width" => 480, "height" => 270);
		$theURL = "http://www.youtube.com/oembed?url=http://www.youtube.com/watch?v=$my_id&format=json";
		$headers = $this->zon_getURL($theURL);
		$myArray = json_decode($headers, true);
		if (strpos($headers, "Our systems have detected") !== false) {
			$myArray["html"] = $my_id;
			exit($headers);
		}
		if (!$myArray || !array_key_exists("html", $myArray))
			return array();
		$myArray["id"] = $my_id;
		$myArray["url"] = "https://www.youtube.com/watch?v=" . $my_id;
		$myArray["html"] = '<iframe width="' . $pra["width"] . '" height="' . $pra["height"] . '" src="https://www.youtube.com/embed/' . $my_id . '?feature=oembed"  allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>';
		return $myArray;
	}
	function zon_getURL($url, $proxy = null, $timeout = 10)
	{
		$ch      = curl_init();
	...
	function getElementsByClassName($dom, $ClassName, $tagName = null, $attr = "class", $match = false)
	{
		...
		for ($i = 0; $i < $Elements->length; $i++) {
			if ($Elements[$i]->getAttribute($attr)) {
				if (
					strpos($Elements[$i]->getAttribute($attr), $ClassName) !== false && $attr == "class" && !$match
					|| $Elements[$i]->getAttribute($attr) == $ClassName
					|| $ClassName == "" && $Elements[$i]->getAttribute($attr)
				) {
					$Matched[] = $Elements[$i];
				}
			}
	...
	function getData($tp = "", $where = [], $w = "*")
	{
		...
		if (is_array($where) && $where) {
			...
			foreach ($where as $key => $value) {
				//if (!in_array($key, ["content", "active"]))
				$w2 .= ($w2 ? " and `" : " `") . $key . "`= '" . $value . "'";
				//if ($key == "content")
				//	$w2 .= ($w2 ? " and `" : " `") . $key . "`!= ''";
			}
	...
	function getproductimages($param)
	{
		...
		if (array_key_exists('product', $param)) {
			...
			$condition .= " and product = $param[product]";
		}
	...
	function getaboutItem($param = array())
	{
	...
	function getHtml($url)
	{
		...
		$dom->loadHTML($html[1]);
		$d = $dom->getElementById("ctl01_PageZoneContainer1");
		if (!$d)
			return "";
		return $this->DOMinnerHTML($d);
	}
	function getPage($url, $type = false)
	{
		$timeout = 60;
		$ad = [
			...
			"User-Agent" => "Mozilla/5.0 (Linux; Android 4.3; Nexus 7 Build/JSS15Q) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.75 Safari/537.36",
		];
		$header = array_map(function ($v) use ($ad) {
			return $v . ": " . $ad[$v];
		}, array_keys($ad));
		$file_get_contents = file_get_contents($url, false, stream_context_create(
			array(
				'http' => array(
					'ignore_errors' => true,
					'method' => 'GET',
		...
		$mime_type = $file_info->buffer($file_get_contents);
		//echo $mime_type;
		if ($type) {
			return [$mime_type == "text/html", $file_get_contents];
		}
		return $file_get_contents;
	}
	function CheckSQl($tp = "", $data = [], $up = true)
	{
		$c = $data;
		unset($c["level"]);
		unset($c["menu"]);
		...
		if ($d) {
			if ($up) {
				//unset($data["level"]);
				$this->UpDate($tp, $data, null, "where id=" . $d[0]["id"]);
			}
			return $d[0]["id"];
		} else {
			// print_r($c);
	...
	function getPageUrl($key, $app = false)
	{
		$php = true;
		if (is_array($key)) {
			if (isset($key["link"]) && $key["link"])
				return $key["link"];
			return ($app ? $app : "page") . ".php?id=" . $key["id"];
		}

		if ($app)
			return $this->getBlogUrl($key, $app);
		if (is_int($key))
			return "page.php?id=" . $key;
		else {
			return "$key.php";
		}
		...
		//	$slug = $titlee["titlearabic"];
		//return $key."_".$this->getSlug(strtolower($contents[0]["title"]));
		return $key . ".php";
		return $key . "_" . $this->getSlug(strtolower($slug));
	}
	...
	function getproducts($parm)
	{
		...
		if (array_key_exists('brand', $parm)) {
			$parm["brand"] = (int)$parm["brand"];
			$condition .= " and brand = $parm[brand]";
		}
		...
		while ($columns = $result->fetch_array(MYSQLI_ASSOC)) {
			...
			$gall["image"] = $columns["image"];
			$gall["full_image"] = $columns["full_image"];
